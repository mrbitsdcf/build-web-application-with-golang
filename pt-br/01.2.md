...
No Windows você precisa criar uma nova variável de ambiente chamada GOPATH, e configurá-la para o diretório de trabalho, como por exemplo: `c:\mygo`
...
Seguindo as instruções acima, execute os comandos abaixo:
...
Agora o diretório e código fonte do nosso pacote foi criado. Conforme explicado, recomendo que utilize o nome do pacote correspondendo ao diretório.
...
Após compilado, podemos ver o arquivo executável do pacote na seguinte pasta:
	...
	// O arquivo do pacote foi gerado com a extenção .a
	mymath.a
...
Para compilar nossa aplicação, deveremos alterar o código fonte de diretório, nesse caso para `$GOPATH/src/mathapp` e executar o comando `go install`. Agora teremos um arquivo executável dentro da pasta `$GOPATH/bin/`. Para executarmos basta digitarmos `./mathapp` no terminal. Você deverá ver o seguinte retorno:
...
Antes de utilizar o `go get` certifique-se de ter instalado ao menos uma delas.
...
O comando `go get` clona o código fonte e dependências para o seu $GOPATH/src e instala os pacote com `go install`
...
Agora que esclarecemos como Go gerencia dependências e código fonte, vamos recapitular de forma rápida a estrutura:

- `bin` contém os executaveis;
- `pkg` contém as bibliotecas ou pacotes compilados
- `src` contém o código fonte da sua aplicação e pacotes.
