...
Nesta sessão, vamos ensinar você como definir constantes, variáveis com tipos elementares e alguns skills em programação Go.

## Definindo Variáveis

Há muitas formas de sintaze que podem ser usadas para definir variáveis em Go.

A palavra-chave `var` é a forma básica de definir variáveis. Note que Go coloca o tipo da variável `depois` de seu nome.

	// define uma variável com nome "variableName" e tipo "type"
	var variableName type

Definindo múltiplas variáveis.

	// define três variáveis cujos tipos são "type"
	var vname1, vname2, vname3 type

Definindo uma variável com valor inicial.

	// define uma variável com nome “variableName”, tipo "type" e valor "value"
	var variableName type = value

Definindo múltiplas variáveis com valores iniciais.

	/*
    Define três variáveis com "type", e inicializa seus valores.
    vname1 é v1, vname2 é v2, vname3 é v3
	*/
	var vname1, vname2, vname3 type = v1, v2, v3

Você acha que é tedioso definir variáveis usando as maneiras acima? Não se preocupe, porque o time de desenvolvedore do Go também acha isso um problema. Portanto, se você quer definir variáveis com valores iniciais, podemos simplesmente omitir seu tipo, então o código será parecido com isto:
...
Certo, eu sei que isso ainda não é suficientemente simples para você. Então vamos ver como arrumar isso:

	/*
    Define três variáveis sem o tipo "type" e sem a palavra-chave "var", e inicializa seus valores.
    vname1 é v1，vname2 é v2，vname3 é v3
	*/
	vname1, vname2, vname3 := v1, v2, v3

Parece bem melhor. Use `:=` para substituir `var` e `tipo`. Isso é chamado de `atribuição breve` (brief statemente). Mas, espere, há uma limitação: essa forma só pode ser utilizada dentro de funções. Você terá erros de compilação se tentar fazer isso fora do corpo das funções. Portanto, nós costumeiramente usamos `var` para definir varíaveis globais e usamos essa forma de atribuição breve em `var()`.

`-` (blank) é uma varíavel especial. Qualquer valor dado a ela será ignorado. Por exemplo, damos `35` para a variável `b` e descartamos `34`. ( ***Este exemplo somente mostra como isso funciona. Parece inútil aqui porque nós frequentemente usamos esse símbolo quando temos o valor de retorno de uma função.*** )

	_, b := 34, 35

Se você não usa as variáveis que você definiu em seu programa, o compilador dará erros de tempo de compilação. Tente compilar o seguinte código e veja o que acontece.
